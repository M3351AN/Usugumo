...
#define rva(instruction, size) \
  (instruction + size +        \
   *reinterpret_cast<long*>(instruction + (size - sizeof(long))))
...
NTSYSAPI NTSTATUS RtlCreateUserThread(HANDLE, PVOID, BOOLEAN, ULONG, SIZE_T,
                                      SIZE_T, PVOID, PVOID, PHANDLE,
                                      PCLIENT_ID);

__declspec(dllimport) NTSTATUS ZwWaitForMultipleObjects(unsigned long, HANDLE[],
                                                        WAIT_TYPE, BOOLEAN,
                                                        LARGE_INTEGER*);

__declspec(dllimport) PPEB PsGetProcessPeb(PEPROCESS);

__declspec(dllimport) NTSTATUS __stdcall ZwQuerySystemInformation(
    SYSTEM_INFORMATION_CLASS, void*, unsigned long, unsigned long*);
...
extern "C" {
#endif
ULONG_PTR __security_cookie = 0x1145141919810;
